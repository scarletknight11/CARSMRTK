...
#region Export functionality
		...
		public static unsafe void SaveAvatarMesh(SkinnedMeshRenderer headMeshRenderer, string avatarId, string outputMeshFile, MeshFileFormat format = MeshFileFormat.OBJ,
			...
			string textureName = null, bool saveTextures = true)
		{
			...
			List<string> pbrTextures = FindExistedPBRTextures(avatarId);

			IMeshConverter meshConverter = CreateMeshConverter();
			IntPtr headMesh = meshConverter.CreateMeshObject(plyFile, "", srcTextureFile, pbrTextures);
			...
			if (format == MeshFileFormat.FBX)
				res = meshConverter.SaveMeshToFbx(headMesh, outputMeshFile, saveTextures ? dstTextureFile : string.Empty);
			else
		...
		public unsafe static void HaircutPlyToFbx(string avatarId, string haircutId, string fbxFile, 
			bool recolorTexture = false, Color color = new Color(), Vector4 tint = new Vector4(), bool saveTextures = true)
		{
			...
			int res = meshConverter.SaveMeshToFbx(haircutMesh, fbxFile, saveTextures ? dstTextureFile : string.Empty);
			if (res != 0)
		...
		private static List<string> FindExistedPBRTextures(string avatarId)
		{
			List<string> pbrTextures = new List<string>();
			var storage = AvatarSdkMgr.Storage();
			List<AvatarFile> pbrTexturesFiles = new List<AvatarFile>() { AvatarFile.METALLIC_MAP_TEXTURE, AvatarFile.ROUGHNESS_MAP_TEXTURE };
