...
namespace ItSeez3D.AvatarSdk.Core
{
	...
	public interface IMeshConverter
	{
		...
		IntPtr CreateMeshObject(string plyModelFile, string templateModelFile, string textureFile, List<string> additionalTextures = null);
	...
	public class CoreMeshConverter : IMeshConverter
	{
		...
		[DllImport(DllHelperCore.dll)]
		private static extern int loadAdditionalTextureForMeshObject(IntPtr mesh, string textureFile);

		public virtual IntPtr CreateMeshObject(string plyModelFile, string templateModelFile, string textureFile, List<string> additionalTextures = null)
		{
			IntPtr meshPtr = createMeshObject(plyModelFile, templateModelFile, textureFile);
			if (meshPtr == IntPtr.Zero)
			{
				Debug.LogErrorFormat("Unable to create mesh object from file: {0}", plyModelFile);
				return meshPtr;
			}

			if (additionalTextures != null)
			{
				foreach (string texturePath in additionalTextures)
					loadAdditionalTextureForMeshObject(meshPtr, texturePath);
			}
