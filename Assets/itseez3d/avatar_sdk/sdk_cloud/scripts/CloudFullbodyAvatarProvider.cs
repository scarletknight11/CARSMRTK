...
namespace ItSeez3D.AvatarSdk.Cloud
{
	public class CloudFullbodyAvatarProvider : CloudAvatarProvider, IFullbodyAvatarProvider
	{
			...
			string name = "name", string description = "")
		{
			...
			AvatarSdkMgr.SpawnCoroutine(InitializeFullbodyAvatarFunc(photoBytes, null, computationParameters, name, description, request));
			return request;
		}

		public AsyncRequest<string> InitializeFullbodyAvatarAsync(string selfieCode, byte[] selfiePhotoBytes, FullbodyAvatarComputationParameters computationParameters, 
			string name = "name", string description = "")
		{
			var request = new AsyncRequest<string>(AvatarSdkMgr.Str(Strings.InitializingAvatar));
			AvatarSdkMgr.SpawnCoroutine(InitializeFullbodyAvatarFunc(selfiePhotoBytes, selfieCode, computationParameters, name, description, request));
			return request;
		}
		...
		public AsyncRequest<FullbodyAvatarComputationParameters> GetAvailableComputationParametersAsync()
		{
			PipelineType pipelineType = PipelineType.FULLBODY;
		...
		private IEnumerator InitializeFullbodyAvatarFunc(byte[] photoBytes, string selfieCode, FullbodyAvatarComputationParameters computationParameters, 
